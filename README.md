...
- **我的** — 分类统计（饼图/趋势/词云）、同步中心、AI 每日复盘、当年今日、应用日志、数据备份

其他能力：Google 日历事件双向同步、Android 桌面小组件、桌面端键盘快捷键、全局搜索、AI 每日复盘与多轮对话。
...
lib/
  ...
  models/                数据模型（时间块、分类、打卡、目标、出行、日记、同步状态等）
  screens/               页面（22 个）
  services/              业务服务（57 个）：Google 日历、Git 同步、打卡、AI、语音、日志等
  widgets/               可复用组件（17 个）
...
`test/` 含 70 个 dart 测试文件，覆盖同步合并、语音解析、日历解析、桌面适配、日志系统、备份回滚、身份隔离等核心逻辑。
