          ...
          children: [
            ...
            Flexible(
              child: ListView(
                ...
                shrinkWrap: true,
                children: [
                  for (final entry in entries) _YearCard(entry: entry),
                ],
              ),
            ),
          ],
...
class _YearCard extends StatelessWidget {
  ...
  const _YearCard({required this.entry});
            ...
            Row(
              children: [
                ...
                if (entry.totalMinutes > 0)
                  Text(
                    '共 ${_formatDuration(entry.totalMinutes)}',
                    style: theme.textTheme.bodySmall?.copyWith(
                      color: theme.colorScheme.onSurfaceVariant,
                    ),
                  ),
...
String _formatDurationCompact(int minutes) {
  if (minutes >= 60) {
    final h = minutes ~/ 60;
    final m = minutes % 60;
...
class _ActivityChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(
        '$label ${_formatDurationCompact(minutes)}',
        style: theme.textTheme.bodySmall?.copyWith(
